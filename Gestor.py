...
class App:
	def __init__(self):
		...
		self.lbl_info = tk.Label(self.root, textvariable = self.text_info , width=114, relief="ridge",bg="#e1f3fd",fg="blue", bd=4)
		self.lbl_info.place(x=47,y=240)

		# -- Nueva ventana dentro de la raiz(root)
		self.frame1 = tk.Frame(self.root,bg="lightgrey",relief="ridge",bd=4)
		...
		self.btn_agregar= tk.Button(self.frame1,text="AGREGAR", width=29, bd=3, command= self.modo_agregar_afiliado)
		self.btn_agregar.place(x=26,y=10) 

		self.btn_editar= tk.Button(self.frame1,text="EDITAR", width=29, bd=3, command= self.modo_editar_afiliado)
		self.btn_editar.place(x=26,y=45)

		self.btn_eliminar= tk.Button(self.frame1,text="ELIMINAR", width=29, bd=3, command= self.modo_eliminar_afiliado)
		self.btn_eliminar.place(x=26,y=80)

		self.lbl_obra_social= tk.Label(self.frame1,text="OBRA SOCIAL", relief= "ridge",width=14)
		...
		self.lbl_info_frame1 = tk.Label(self.frame1,textvariable = self.text_info_frame1 , relief= "ridge",width=29, bg="#e1f3fd", fg="black")
		...
		self.btn_crear_doc= tk.Button(self.frame1,text="GENERAR DOCUMENTO",width=29, bd=3, command= self.generar_documento)
		self.btn_crear_doc.place(x=562,y=10)

		self.btn_carpeta_doc= tk.Button(self.frame1,text="EXAMINAR DOCUMENTOS",width=29, bd=3, command= self.abrir_carpeta_docs)
		self.btn_carpeta_doc.place(x=562,y=45)

		self.root.bind("<<ComboboxSelected>>", lambda _: self.actualizar_tabla() )
	...
	def crear_tabla(self,filas=10): # --- da formato a la tabla (treeview)
		columnas = ["APELLIDO","NOMBRE","DNI/AFILIADO","TELEFONO","OBRA SOCIAL"]
		self.tabla = ttk.Treeview(self.root, height=filas,columns=(columnas))
		...
		# --- barra scroll
		self.scroll = tk.Scrollbar(self.root, orient="vertical", command=self.tabla.yview)
		self.scroll.place(x=860, y=10, height=220)
		self.tabla.configure(yscrollcommand=self.scroll.set)

		# --- formato a las columnas
		self.tabla.column("#0", width=0, stretch= tk.NO , minwidth=100)
		self.tabla.column("APELLIDO", anchor= tk.W, width=200, minwidth = 200)
		self.tabla.column("NOMBRE", anchor= tk.W, width=200, minwidth = 200)
		self.tabla.column("DNI/AFILIADO", anchor= tk.E, width=150, minwidth = 150)
		self.tabla.column("TELEFONO", anchor= tk.E, width=150, minwidth = 150)
		self.tabla.column("OBRA SOCIAL", anchor= tk.CENTER, width=100,minwidth = 100)
		...
		self.tabla.heading("#3", text="DNI/AFILIADO", anchor= tk.CENTER)
		self.tabla.heading("#4", text="TELEFONO", anchor= tk.CENTER)
		self.tabla.heading("#5", text="OBRA SOCIAL", anchor= tk.CENTER)
	...
	def abrir_ventana(self): # --- configura ventana para entrada de datos
		...
		self.lbl_apellido = tk.Label(self.top_level, text = "APELLIDO", width=20, relief="ridge", bg="#e1f3fd")
		self.lbl_apellido.place(x=10,y=10)
		self.entry_apellido = tk.Entry(self.top_level,textvariable = self.apellido,width=30)
		self.entry_apellido.place(x=170,y=10)

		self.lbl_nombre = tk.Label(self.top_level, text = "NOMBRE", width=20, relief="ridge", bg="#e1f3fd")
		self.lbl_nombre.place(x=10,y=40)
		self.entry_nombre = tk.Entry(self.top_level,textvariable = self.nombre,width=30)
		self.entry_nombre.place(x=170,y=40) 

		self.lbl_dni_afiliado = tk.Label(self.top_level, text = "DNI/AFILIADO", width=20, relief="ridge", bg="#e1f3fd")
		self.lbl_dni_afiliado.place(x=10,y=70)
		self.lbl_dni_afiliado= tk.Entry(self.top_level,textvariable = self.dni_afiliado,width=30)
		self.lbl_dni_afiliado.place(x=170,y=70)

		self.lbl_telefono = tk.Label(self.top_level, text = "TELEFONO", width=20, relief="ridge", bg="#e1f3fd")
		self.lbl_telefono.place(x=10,y=100)
		self.entry_telefono = tk.Entry(self.top_level,textvariable = self.telefono,width=30)
		self.entry_telefono.place(x=170,y=100)

		self.lbl_obra_social = tk.Label(self.top_level, text = "OBRA SOCIAL", width=20, relief="ridge", bg="#e1f3fd")
		...
		self.btn_ingresar= tk.Button(self.top_level,text="GRABAR DATOS",bd=3, width=15, command= self.grabar_afiliado)
		self.btn_ingresar.place(x=10,y=175)

		self.btn_salir= tk.Button(self.top_level,text="SALIR", bd=3, width=15, command= self.salir_ventana)
	...
	def cargar_tabla(self,lista_afiliados,obrasocial="TODAS"): # --- agrega datos a la tabla (treeview)
		for afiliado in lista_afiliados: 
			if obrasocial != "TODAS":
				...
				self.tabla.insert("", tk.END, text="", values=(afiliado[0],afiliado[1], afiliado[2], afiliado[3],afiliado[4]))
			else:
				self.tabla.insert("", tk.END, text="", values=(afiliado[0],afiliado[1], afiliado[2], afiliado[3],afiliado[4]))
		self.info_num_afiliados()
	...
	def modo_editar_afiliado(self): #--- permite editar valores de la fila seleccion
		...
		print(self.afiliado_target)
		if not self.afiliado_target:
	...
	def modo_eliminar_afiliado(self):
		...
		self.afiliado_target = self.get_datos_tabla() 
		if not self.afiliado_target :
			messagebox.showinfo(message="POR FAVOR SELECCIONE ALGUN AFILIADO",
					   title="ELIMINAR AFILIADO") # --- caja de mensaje
		else:
			if  messagebox.askyesno(message="ESTA ACCIÓN NO PODRÁ DESHACERSE\n\n            ¿DESEA CONTINUAR?",
						   title=f"{(self.modo)} AFILIADO"): 
	...
	def eliminar_afiliado(self,archivo="afiliados/afiliados.csv"):
		try:
			...
			messagebox.showerror(message=e, title="ERROR!!!")
		try:
			with open(archivo, 'w', encoding='latin1',newline='') as datos:
				csvwriter = csv.writer(datos)
				for linea in nueva_lista_afiliados:
					if linea == self.afiliado_target:
						continue
